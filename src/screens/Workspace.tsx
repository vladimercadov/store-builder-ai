      ...
      <div className="flex-1 relative overflow-hidden flex items-center justify-center p-8 bg-surface-dim">
        ...
        >
          {baseStoreImageUrl ? (
            ...
            <div className="absolute inset-0 bg-surface-bright flex items-center justify-center text-on-surface-variant font-mono text-sm uppercase tracking-tighter">
              Esperando captura de local...
            </div>
          )}
          ...
          <div className="absolute bottom-6 left-6 flex items-center gap-3 bg-surface/90 border border-outline p-2 px-4 rounded-full backdrop-blur-md">
            ...
            <span className="text-[10px] font-mono font-bold text-on-surface uppercase tracking-widest">
              Engine: {baseStoreImageUrl ? 'Profundidad Activa 1:1' : 'Calibrando...'}
            </span>
          </div>
        </div>
      </div>
      ...
      <aside className="w-80 border-l border-outline-variant bg-surface flex flex-col overflow-y-auto">
        ...
        {selectedPlacedAsset ? (
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
                ...
                <div className="mt-4 space-y-2 border-t border-primary/10 pt-4">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="font-bold text-on-surface-variant uppercase">Inversión:</span>
                    <span className="font-black text-primary text-sm">{formatCurrencyCOP(selectedPlacedAsset.priceCOP)}</span>
                  </div>
                </div>
              </div>
              <button onClick={() => setSelectedAssetId(null)} className="w-full py-2 bg-surface-bright border border-outline-variant rounded-lg text-[10px] font-bold uppercase hover:bg-surface">
                Liberar Selección
              </button>
           </div>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center opacity-40">
            ...
            <p className="text-[9px] uppercase font-bold tracking-widest text-on-surface-variant">Motor de Diseño Activo</p>
          </div>
        )}
      </aside>
    </div>
